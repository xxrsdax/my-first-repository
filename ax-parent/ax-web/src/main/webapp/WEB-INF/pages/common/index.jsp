...
<script type="text/javascript">
	...
	$(function() {
        ...
        $("#btnEp").click(function () {
            //进行表单校验
            var v = $("#editPasswordForm").form("validate"); //调用插件，进行表单校验
            //判断是否合法
            if(v) {
                ...
                if(v1 == v2 ) {
                    //ajax发送请求，进行修改密码
                    $.post(
                        "UserAction_editPassword.action", //url
                        {"password":v1}, //注意格式 ，自己写错过
                        function (data) {

                            if(data == "1") {
                                //修改成功关闭窗口
                                $("#editPwdWindow").window("close");//修改成功，关闭窗口
                            }else{
                                //修改失败，给出提示信息
                                $.messager.alert("提示信息","密码修改失败！","error");
                            }

                        }   //后面的还有一个参数被省略 “json” ,表示回显的data不一定是json格式的数据
                    );
                }else {
                    //显示错误信息的提示
                    $.messager.alert("提示信息","两次输入不一致","warning");
                }
            }
        });
	});
...
<body class="easyui-layout">
	<div data-options="region:'north',border:false"
		...
		<div id="layout_north_kzmbMenu" style="width: 100px; display: none;">
			...
			<div onclick="logoutFun();">退出系统</div>  <%--退出系统--%>
            ...
            <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
              <form id="editPasswordForm">
                <table cellpadding=3>
                    <tr>
                        <td>新密码：</td>
									<%--  设置为非空选项，  长度设置为： 6 到 16  位  --%>
                        <td><input  required="true" data-options="validType:'length[6,16]'" id="txtNewPass" type="Password" class="txt01 easyui-validatebox" /></td>
                    </tr>
                    <tr>
                        <td>确认密码：</td>
                        <td><input required="true"  data-options="validType:'length[6,16]'"id="txtRePass" type="Password" class="txt01 easyui-validatebox" /></td>
                    </tr>
                </table>
			  </form>
            </div>
